...
instance (Argo.HasCodec a, Argo.HasCodec b, Argo.HasCodec c, Argo.HasCodec d) => Argo.HasCodec (a, b, c, d) where
  codec =
    Argo.Codec.Array.fromArrayCodec Argo.Type.Permission.Forbid
      $ (,,,)
      <$> Argo.Codec.Codec.project
            (\(x, _, _, _) -> x)
            (Argo.Codec.Array.element Argo.codec)
      <*> Argo.Codec.Codec.project
            (\(_, x, _, _) -> x)
            (Argo.Codec.Array.element Argo.codec)
      <*> Argo.Codec.Codec.project
...
instance Argo.HasCodec a => Argo.HasCodec (Data.Map.Map Data.Text.Text a) where
  codec = Argo.Codec.Codec.map
    (Data.Map.mapKeys (\(Argo.Name x) -> x))
    (Data.Map.mapKeys Argo.Name)
    Argo.codec
