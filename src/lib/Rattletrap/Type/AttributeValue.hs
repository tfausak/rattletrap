...
instance Argo.HasCodec AttributeValue where
  codec = Argo.identified $ Argo.oneOf
    ...
    $ Argo.required id "applied_damage"
    , Argo.mapMaybe
      (Just . Boolean)
      (\x -> case x of
        Boolean y -> Just y
